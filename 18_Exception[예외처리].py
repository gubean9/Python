...
# # 예외 처리
# # try / except
# # 프로그램을 실행할 때 개발자가 미연에 알아차리지 못한 오류나 사용자의 예외 데이터 입력 상황, 통신 규약에 맞지 않는 데이터의 전송 등의 예외 상황을 유연하게 대처하고 프로그램의 유지 보수를 도와줌
# # 사용자에게 시스템 오류를 노출하지 않고 올바르게 로직을 처리할 수 있게하여 프로그램의 신뢰성을 높이고, 사용자에게 올바른 사용을 유도할 수 있다
...
# # except의 종류
# # Exception : 예외 상황의 최상위 클래스(모든 예외상황을 검출할 수 있다)
# ivalue = input("점수를 입력하세요")
# score = 0
# try:
#     # 입력받은 내용을 정수로 형변환
...
#     # 입력받은 내용을 정수로 형변환
#     score = int(ivalue)
# except TypeError: # 데이터 타입 형변환에 실패하였을 경우
#     print("타입 형변환에 실패")
#     score = 20
...
try:
    ...
    assert score <= 100, "102행 : 점수 범위 초과"
except ValueError:
    print("타입형변환에 실패")
    print("데이터 복구 rollback")
    score = 20
